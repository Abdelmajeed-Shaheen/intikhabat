...
class CreateComitteeForm(forms.ModelForm):
    ...
    phone = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control','placeholder':'رقم هاتف اللجنة'}))
...
class ComitteeTaskForm(forms.ModelForm):
    ...
    description=forms.CharField(widget=forms.Textarea(attrs={'class':'form-control','placeholder':'موضوع المهمة'}))
    notes=forms.CharField(widget=forms.Textarea(attrs={'class':'form-control','placeholder':'التوصيات'}))
